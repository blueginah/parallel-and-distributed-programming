 ...
 * Usage:    ./my_pth_ll_rwl <thread_count>
...
#  endif
   ...
   //thread_handles에 저장이 되어 있어 pthread들이!!
   pthread_mutex_init(&count_mutex, NULL);

   //@@
   pthread_cond_init(&rwcv, NULL);
...
void* Thread_work(void* rank) {
      ...
      if (which_op < search_percent) {
        ...
        ///READER!
         ...
         while(writers_count != 0)
             pthread_cond_wait(&rwcv, &readers_mutex);
         readers_count++;
         ...
         while(readers_count != 0 && writers_count != 0)
             pthread_cond_wait(&rwcv, &writers_mutex);
         writers_count++;
   ...
   //count_mutex tells how many operations are there for each functions
